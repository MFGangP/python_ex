...
# 리스트에서 요소를 찾을 떄 해당 요소의 인덱스를 알고자 한다면 index()를 사용하면 된다.
...
# 2번째 : 리스트명.remove("요소"), 리스트에 해당하는 요소가 있으면 삭제한 후, 요소를 반환 안한다.
...
print("---------------------")
hero = ["스파이더맨", "배트맨", "슈퍼맨", "아이언맨", "헐크", "태권V", "배트맨"]
# 아래와 같이 중복된 요소를 한번에 삭제를 하고 싶다면 반복문을 이용하여 제거하도록 하자.
value = "배트맨"
while value in hero:
    hero.remove("배트맨")
print("hero : ", hero)
...
# del 키워드로 요소를 삭제하기
hero = ["스파이더맨", "배트맨", "슈퍼맨", "아이언맨", "헐크", "태권V", "배트맨"]
del hero[0:2]   # 좌측과 같은 결과를 나타내는 것이 clear() 동일하다.
print("hero : ", hero)
