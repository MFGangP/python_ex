...
# 손쉽게 처리할 수 있는 형태의 데이터 저장 구조를 의미한다.
...
# 리스트 순회해서 출력하기(첫 번째 방법 for 문과 range()를 이용)
# for 문은 리스트와 궁합이 잘 맞다.
print("----------------")
for i in range(len(scores)):
    print(i, scores[i])

# 리스트의 값을 순회해서 바꾸기(두 분쨰 방법 for 문과 리스트 시퀀스를 사용)
print("----------------")
for i in range(len(scores)):
    # scores[i] = "안녕"
    scores[i] = i + 10
 ...
 # 두 번째 방법을 사용할 때는 인덱스를 잘 생각해야한다.
print("----------------")
for i in scores:
    # scores[i] = "안녕"
    # 아래와 같이 코드를 작성하면 인덱스가 초과되어서 에러를 발생시킨다.
    # IndexError 발생함. 저장할 방이 없는데 어떻게 저장을 해야되나.
    # scores[i] = i + 10
    print(i)

# 사용자로 부터 입력을 받아서 리스트에 값을 저장하는 코드
# print("----------------")
# nums = []
# for i in range(5):
...
for i in range(len(li2)):       # li2는 주소의 주소를 가지고 있다. len(li2) = 3이다.
    for j in range(len(li2[i])):    # li2[0],li2[1],li2[2] 도 주소값을 지니고 있다.
        print(li2[i][j])        # 이중 내장리스트는 li[i][j]은 곧 변수와 동일하다.
