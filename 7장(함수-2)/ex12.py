...
# 무명함수는 함수 명이 없고, 구현부만 존재한다. 파이썬에서는 무명함수를 만들 때
# lambda 키워드로 만들 수 있다
# 무명함수는 print()등과 같은 일반 함수를 호출할 수가 없다. 오직 계산만 가능하다.
# 전역변수를 참조할 수가 없다.
...
def main():
    ...
    # 가장 많이 사용 되는 곳은 GUI 프로그램에서 이벤트를 처리하는 콜백 함수 형태로
    # 많이 사용된다.

    sum = lambda x, y: x + y
    print("람다식을 이용한 함수명 sum의 결과 : ", sum(10, 50))
    print("람다식을 이용한 함수명 sum의 결과 : ", sum(10, 150))
    # 람다식으로 구성되어진 리스트 데이터
    li = [lambda x: x ** 2, lambda x: x ** 3, lambda x: x ** 4, ]
    for f in li:
        print(f(10))
    # 람다 함수를 출력을 하면 결과론으로 함수 객체를 출력하는 형태가 된다.
